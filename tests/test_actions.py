...
# 1) Bypass success
def test_try_bypass_success():
    state = init_state()
    fe = FailureEvent(comp_id="C03", op_type="unscrew", tool_id=None, symptom=Symptom.ACCESS_BLOCKED,
                      signals={}, context={"alt_paths": True}, history={}, timestamp="2025-09-16T00:00:00Z")
    res = try_bypass(state, fe)
    assert res.success is True
    assert res.updates.get("skipped") == "C03"
...
# 6) Destroy forbidden
def test_try_destroy_forbidden():
    state = init_state()
    fe = FailureEvent(comp_id="C08", op_type="pry", tool_id=None, symptom=Symptom.LOW_VALUE_BOTTLENECK,
                      signals={}, context={"no_destroy": True}, history={}, timestamp="2025-09-16T00:00:00Z")
    res = try_destroy(state, fe)
    assert res.success is False
...
def test_replan_grasp_fail(monkeypatch):
    state = init_state()
    monkeypatch.setattr("src.adaptive.actions.replan_with_overlay", lambda state_arg: [])
    res = replan_grasp(state)
    assert res.success is False
